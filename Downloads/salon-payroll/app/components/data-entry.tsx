...
export default function DataEntry({ employees, payrollData, onPayrollDataChange, selectedMonth }: DataEntryProps) {
  ...
  const handleSaveData = () => {
    ...
    const newData: PayrollData = {
      ...
      serviceRevenue: editingData.serviceRevenue || 0,
      productRevenue: editingData.productRevenue || 0,
      kpiAchieved: editingData.kpiAchieved || false,
      bonus: editingData.bonus || 0,
      penalty: editingData.penalty || 0,
      advance: editingData.advance || 0,
      dailyRevenue: editingData.dailyRevenue || 0,
      monthlyRevenue: editingData.monthlyRevenue || 0,
      facialRevenue: editingData.facialRevenue || 0,
      washingRevenue: editingData.washingRevenue || 0,
      chemicalRevenue: editingData.chemicalRevenue || 0,
      bleachingRevenue: editingData.bleachingRevenue || 0,
    }

    if (editingData.id) {
      onPayrollDataChange(payrollData.map((data) => (data.id === editingData.id ? newData : data)))
    } else {
      onPayrollDataChange([...payrollData, newData])
    }
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setEditingData({ ...editingData, kpiAchieved: checked as boolean })}
                    />
        ...
        <TabsContent value="import">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Tải lên file Excel chứa dữ liệu lương nhân viên</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                <Upload className="h-12 w-12 mx-auto text-gray-400 mb-4" />
                <p className="text-lg font-medium text-gray-900 mb-2">Kéo thả file Excel vào đây</p>
                <p className="text-gray-500 mb-4">hoặc</p>
                <Button variant="outline">Chọn file Excel</Button>
                <p className="text-sm text-gray-500 mt-4">Hỗ trợ file .xlsx, .xls (tối đa 10MB)</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
